...
assert.ok(comparisonHtml.includes('data-ai-accordion-section'), 'AI options modal uses stacked accordion sections');
assert.ok(comparisonHtml.includes('data-ai-accordion-trigger'), 'AI options modal exposes accordion triggers');
assert.ok(comparisonHtml.includes('class="ai-accordion-chevron" aria-hidden="true"><svg'), 'accordion triggers use circular SVG chevrons');
...
assert.ok(!comparisonHtml.includes('class="ai-accordion-section active"'), 'top-level AI expand/collapse sections start collapsed by default');
...
assert.ok(/\.ai-options-body\s*\{[^}]*flex-direction:\s*column/.test(comparisonHtml), 'AI options modal stacks accordion sections vertically');
assert.ok(/\.ai-accordion-section\s*\{[^}]*\/\* border: 1px solid var\(--rule\); \*\//.test(comparisonHtml), 'accordion section uses the requested unbordered shell rule');
assert.ok(/\.ai-accordion-chevron\s*\{[^}]*border-radius:\s*999px/.test(comparisonHtml), 'accordion chevron is circular');
assert.ok(/\.ai-accordion-panel\s*\{[^}]*display:\s*none/.test(comparisonHtml), 'accordion panels collapse by default');
assert.ok(/\.ai-accordion-section\.active \.ai-accordion-panel\s*\{[^}]*display:\s*block/.test(comparisonHtml), 'active accordion panel opens');
...
assert.ok(comparisonJs.includes('function toggleAiAccordionSection'), 'comparison script toggles expand/collapse sections independently');
assert.ok(!comparisonJs.includes('openAiAccordionSection(document.querySelector'), 'AI modal does not force-open the first section by default');
assert.ok(/\.ai-field-grid\s*\{[^}]*grid-template-columns:\s*repeat\(4, minmax\(0, 1fr\)\)/.test(comparisonHtml), 'field selection renders in four columns');
