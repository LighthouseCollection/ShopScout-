...
for (const [id, feature] of Object.entries(vocab.features)) {
  ...
  for (const entry of feature.vocabulary) {
    ...
    for (const alias of entry.aliases) {
      ...
      assert.ok(!/[;|]|,\s*[\p{L}]/u.test(alias),
        'alias is a single normalized value, not an unsplit multi-value list: ' + alias);
    }
  }
}
...
assert.strictEqual(
  vocab.source.generator,
  'scripts/build-normalization-libraries/build-icecat-vocabulary.js',
  'icecatVocabulary is generated by the real Icecat vocabulary generator'
);
assert.ok(vocab.source.generatorVersion >= 1,
  'icecatVocabulary real generator version is at least 1');
assert.ok(isSha256(vocab.source.sourceSha256) && !/^0+$/.test(vocab.source.sourceSha256),
  'icecatVocabulary sourceSha256 is a real fingerprint');
const vocabEntryCount = Object.values(vocab.features).reduce(
  (n, f) => n + f.vocabulary.length,
  0
);
assert.ok(vocabKeys.length >= 20,
  'real Icecat vocabulary emits at least 20 features');
assert.ok(vocabEntryCount >= 100,
  'real Icecat vocabulary emits at least 100 vocabulary entries');
...
assert.notStrictEqual(
  ...
  true,
  'manifest does not mark icecatVocabulary.json as a stub'
);
assert.ok(
  manifest.outputs['icecatVocabulary.json'].vocabularyEntryCount >= 100,
  'manifest records real Icecat vocabulary entry count'
);
