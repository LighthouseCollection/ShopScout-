...
assert.ok(js.includes('renderAiSecondOpinionTab'), 'redesigned results include a second opinion tab renderer');
assert.ok(js.includes('const secondOpinionTab = vm.stages.secondOpinion'), 'results view only exposes the second opinion tab when a run has that stage');
assert.ok(js.includes('const secondOpinionPane = vm.stages.secondOpinion'), 'results view only renders the second opinion pane when a run has that stage');
...
assert.ok(js.includes('class="cmp-table"'), 'results view renders redesigned comparison tables');
...
assert.ok(js.includes('class="sortable"'), 'comparison headers use the redesign sortable table affordance');
assert.ok(js.includes('sortComparisonTable'), 'sortable comparison headers have interaction code');
